...
void testarm64()
{
	...
	add(a, x0, sp, x2);                 expect(a, {0xe0, 0x63, 0x22, 0x8b});
	add(a, x0, xzr, x2);                expect(a, {0xe0, 0x03, 0x02, 0x8b});
	add(a, w0, wsp, w2);                expect(a, {0xe0, 0x43, 0x22, 0x0b});
	add(a, x0, sp, xzr);                expect(a, {0xe0, 0x63, 0x3f, 0x8b});
	sub(a, x5, x2, x17, UXTX, 4);       expect(a, {0x45, 0x70, 0x31, 0xcb});
	sub(a, x4, xzr, x17, LSL, 61);      expect(a, {0xe4, 0xf7, 0x11, 0xcb});
	sub(a, x5, x6, 1234, LSL, 12);      expect(a, {0xc5, 0x48, 0x53, 0xd1});
	sub(a, x0, sp, x2);                 expect(a, {0xe0, 0x63, 0x22, 0xcb});
	sub(a, x0, xzr, x2);                expect(a, {0xe0, 0x03, 0x02, 0xcb});
	sub(a, w0, wsp, w2);                expect(a, {0xe0, 0x43, 0x22, 0x4b});
	sub(a, x0, sp, xzr);                expect(a, {0xe0, 0x63, 0x3f, 0xcb});
	sub(a, x17, x8, 4077);              expect(a, {0x11, 0xb5, 0x3f, 0xd1});
label(a, "foo");
...
label(a, "bar");
	...
	bl(a, "foo");                       expect(a, {0xfa, 0xff, 0xff, 0x97});
	br(a, x23);                         expect(a, {0xe0, 0x02, 0x1f, 0xd6});
	blr(a, x15);                        expect(a, {0xe0, 0x01, 0x3f, 0xd6});
	adds(a, x5, x2, x17, UXTX, 4);      expect(a, {0x45, 0x70, 0x31, 0xab});
	adds(a, x4, xzr, x17, LSL, 61);     expect(a, {0xe4, 0xf7, 0x11, 0xab});
	adds(a, x5, x6, 1234, LSL, 12);     expect(a, {0xc5, 0x48, 0x53, 0xb1});
	subs(a, x5, x2, x17, UXTX, 4);      expect(a, {0x45, 0x70, 0x31, 0xeb});
	subs(a, x4, xzr, x17, LSL, 61);     expect(a, {0xe4, 0xf7, 0x11, 0xeb});
	...
	orr(a, x9, xzr, xzr);               expect(a, {0xe9, 0x03, 0x1f, 0xaa});
	orr(a, xzr, x9, xzr);               expect(a, {0x3f, 0x01, 0x1f, 0xaa});
	mov(a, x9, x23);                    expect(a, {0xe9, 0x03, 0x17, 0xaa});
	mov(a, x14, xzr);                   expect(a, {0xee, 0x03, 0x1f, 0xaa});
	mov(a, x28, sp);                    expect(a, {0xfc, 0x03, 0x00, 0x91});
	mov(a, sp, x13);                    expect(a, {0xbf, 0x01, 0x00, 0x91});
	orr(a, x8, x9, 0x1);                expect(a, {0x28, 0x01, 0x40, 0xb2});
	orr(a, w8, w9, 0xf0f0f0f0);         expect(a, {0x28, 0xcd, 0x04, 0x32});
	orr(a, w8, w9, 0xf0f0f0f0);         expect(a, {0x28, 0xcd, 0x04, 0x32});
	orr(a, x8, x9, 0xcfcfcfcfcfcfcfcf); expect(a, {0x28, 0xd5, 0x02, 0xb2});
	clear(a);
}
